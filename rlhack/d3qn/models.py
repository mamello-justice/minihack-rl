...
class BaseNet(NetHackNet):
    ...
    def forward(self, inputs):
        ...
        feat_B = features_emb.shape[0]
        if (torch.is_tensor(feat_B)):
            feat_B = feat_B.item()
        ...
        glyph_B = glyphs_rep.shape[0]
        if (torch.is_tensor(glyph_B)):
            glyph_B = glyph_B.item()
